...
typedef union { /* new defined union for MD05 Op Code Command Data */
	...
	struct {
		uint8_t Num_Of_VDO : 3, Cmd_Type : 2, Reserved : 3;
	} BITS;
} SEND_VDM_BYTE_DATA;
...
typedef union {
	...
	struct {
		uint32_t Number_of_obj : 3, MSG_ID : 3, Port_Power_Role : 1,
			Specification_Rev : 2, Port_Data_Role : 1, Reserved : 1,
			MSG_Type : 4;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t VDM_command : 5, Rsvd2_VDM_header : 1,
			VDM_command_type : 2, Object_Position : 3,
			Rsvd_VDM_header : 2, Structured_VDM_Version : 2,
			VDM_Type : 1, Standard_Vendor_ID : 16;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t USB_Vendor_ID : 16, Rsvd_ID_header : 10,
			Modal_Operation_Supported : 1, Product_Type : 3,
			Data_Capable_USB_Device : 1, Data_Capable_USB_Host : 1;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t Cert_TID : 20, Rsvd_cert_VDOer : 12;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t Device_Version : 16, Product_ID : 16;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t USB_Superspeed_Signaling_Support : 3,
			SOP_contoller_present : 1, Vbus_through_cable : 1,
			...
			Cable_Termination_Type : 2, Cable_Latency : 4,
			TypeC_to_Plug_Receptacle : 1, TypeC_to_ABC : 2,
			Rsvd_CABLE_VDO : 4, Cable_Firmware_Version : 4,
			Cable_HW_Version : 4;
	} BITS;
} UND_CABLE_VDO_Type;
...
typedef union {
	...
	struct {
		uint32_t VENDOR_DEFINED_MESSAGE : 15, VDM_TYPE : 1,
			USB_Vendor_ID : 16;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t DATA : 8, TOTAL_NUMBER_OF_UVDM_SET : 4, RESERVED : 1,
			COMMAND_TYPE : 2, DATA_TYPE : 1, PID : 16;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t ORDER_OF_CURRENT_UVDM_SET : 4, RESERVED : 9,
			COMMAND_TYPE : 2, DATA_TYPE : 1, PID : 16;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t Port_Capability : 2, Signalling_DP : 4,
			Receptacle_Indication : 1, USB_2p0_Not_Used : 1,
			DFP_D_Pin_Assignments : 8, UFP_D_Pin_Assignments : 8,
...
typedef union {
	...
	struct {
		uint32_t Port_Connected : 2, Power_Low : 1, Enabled : 1,
			Multi_Function_Preference : 1,
			USB_Configuration_Req : 1, Exit_DP_Mode_Req : 1,
			HPD_State : 1, HPD_Interrupt : 1, Reserved : 23;
	} BITS;
...
typedef union {
	...
	struct {
		uint32_t SEL_Configuration : 2, Select_DP_V1p3 : 1,
			Select_USB_Gen2 : 1, Select_Reserved_1 : 2,
			Select_Reserved_2 : 2, DFP_D_PIN_Assign_A : 1,
			DFP_D_PIN_Assign_B : 1, DFP_D_PIN_Assign_C : 1,
			DFP_D_PIN_Assign_D : 1, DFP_D_PIN_Assign_E : 1,
			DFP_D_PIN_Assign_F : 1, DFP_D_PIN_Reserved : 2,
			UFP_D_PIN_Assign_A : 1, UFP_D_PIN_Assign_B : 1,
			UFP_D_PIN_Assign_C : 1, UFP_D_PIN_Assign_D : 1,
			UFP_D_PIN_Assign_E : 1, UFP_D_PIN_Assign_F : 1,
			UFP_D_PIN_Reserved : 2, DP_MODE_Reserved : 8;
	} BITS;
...
typedef union sec_uvdm_header {
	...
	struct {
		uint32_t data : 8, total_number_of_uvdm_set : 4, direction : 1,
			command_type : 2, data_type : 1, pid : 16;
	} BITS;
} U_SEC_UVDM_HEADER;
...
typedef union sec_tx_data_header {
	...
	struct {
		uint32_t data_size_of_current_set : 8, total_data_size : 8,
			reserved : 12, order_of_current_uvdm_set : 4;
	} BITS;
...
typedef union sec_data_tx_tailer {
	...
	struct {
		uint32_t checksum : 16, reserved : 16;
	} BITS;
...
typedef union sec_data_rx_header {
	...
	struct {
		uint32_t reserved : 18, result_value : 2,
...
#endif
void max77729_receive_alternate_message(
...
extern void max77729_vdm_process_set_DP_configure_mode_req(void *data,
							   uint8_t W_DATA);
