...
static int
nopmi_chg_jeita_get_bat_temperature(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
...
static int
nopmi_chg_jeita_get_charger_voltage(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {
		0,
	};
...
static int
nopmi_chg_jeita_get_batt_id(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {
		0,
	};
...
static int
nopmi_chg_jeita_get_pd_active(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {
		0,
	};
...
static int nopmi_chg_jeita_get_charger_term_current(
	struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	union power_supply_propval prop = {
		0,
	};
...
static int
nopmi_chg_jeita_set_charger_current(struct nopmi_chg_jeita_st *nopmi_chg_jeita,
				    int cc)
{
	union power_supply_propval prop = {
		0,
...
static int
nopmi_chg_jeita_set_charger_voltage(struct nopmi_chg_jeita_st *nopmi_chg_jeita,
				    int cv)
{
	union power_supply_propval prop = {
		0,
...
static int nopmi_chg_jeita_set_charger_term_current(
	struct nopmi_chg_jeita_st *nopmi_chg_jeita, int term_curr)
{
	union power_supply_propval prop = {
		0,
	};
...
*/
static void
nopmi_chg_handle_jeita_current(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	int ret = 0;
	...
	int sc8551_charge_enable_flag =
		0; //this flag used by jeta:nopmi_chg_jeita.c to set sw_chip fv && used by fg_chip do another soc
	...
	} else if (nopmi_chg_jeita->battery_temp >
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_above_t4_cv;
		else if (sw_jeita->sm == TEMP_T3_TO_T4)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_t3_to_t4_cv;
		else if (sw_jeita->sm == TEMP_T2_TO_T3)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.normal_charge_voltage;
		else if (sw_jeita->sm == TEMP_T1P5_TO_T2)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_t1p5_to_t2_cv;
		else if (sw_jeita->sm == TEMP_T1_TO_T1P5)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_t1_to_t1p5_cv;
		else if (sw_jeita->sm == TEMP_T0_TO_T1)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_t0_to_t1_cv;
		else if (sw_jeita->sm == TEMP_TN1_TO_T0)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_tn1_to_t0_cv;
		else if (sw_jeita->sm == TEMP_BELOW_T0)
			sw_jeita->cv =
				nopmi_chg_jeita->dt.jeita_temp_below_t0_cv;
		else
			sw_jeita->cv =
				nopmi_chg_jeita->dt.normal_charge_voltage;
	} else {
	...
	if (sw_jeita->cv != chg1_cv) {
		if (nopmi_chg_jeita->fv_votable) {
			...
			ret = nopmi_chg_jeita_set_charger_voltage(
				nopmi_chg_jeita, sw_jeita->cv);
			if (ret < 0)
	...
	if (NOPMI_CHARGER_IC_MAXIM != nopmi_get_charger_ic_type()) {
		...
		term_curr_pre = nopmi_chg_jeita_get_charger_term_current(
			nopmi_chg_jeita);
		if (sw_jeita->term_curr != term_curr_pre) {
			ret = nopmi_chg_jeita_set_charger_term_current(
				nopmi_chg_jeita, sw_jeita->term_curr);
			if (ret < 0)
...
{
	nopmi_chg_jeita->battery_temp =
		nopmi_chg_jeita_get_bat_temperature(nopmi_chg_jeita);
	...
	if (nopmi_chg_jeita->sw_jeita->charging == false) {
		...
		if (nopmi_chg_jeita->sw_jeita->can_recharging == true) {
			switch (nopmi_get_charger_ic_type()) {
			case NOPMI_CHARGER_IC_MAXIM:
				vote(nopmi_chg_jeita->chgctrl_votable,
				     JEITA_CHG_VOTER, false,
				     CHG_MODE_CHARGING_OFF);
				break;
			case NOPMI_CHARGER_IC_SYV:
				//nopmi_chg_jeita_set_charger_enabled(nopmi_chg_jeita, true);
				nopmi_set_charge_enable(true);
				break;
			case NOPMI_CHARGER_IC_SC:
				break;
			default:
...
static void
nopmi_chg_jeita_state_init(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	struct sw_jeita_data *sw_jeita = nopmi_chg_jeita->sw_jeita;
...
int nopmi_chg_jeita_init(struct nopmi_chg_jeita_st *nopmi_chg_jeita)
{
	...
	if (!nopmi_chg_jeita->sw_jeita) {
		nopmi_chg_jeita->sw_jeita =
			kmalloc(sizeof(struct sw_jeita_data), GFP_KERNEL);
		if (!nopmi_chg_jeita->sw_jeita) {
	...
	if (NOPMI_CHARGER_IC_MAXIM != nopmi_get_charger_ic_type()) {
		nopmi_chg_jeita->battery_id =
			nopmi_chg_jeita_get_batt_id(nopmi_chg_jeita);
	}

	INIT_DELAYED_WORK(&nopmi_chg_jeita->jeita_work,
			  nopmi_chg_jeita_workfunc);
