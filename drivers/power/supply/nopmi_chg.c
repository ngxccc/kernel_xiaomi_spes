#include "drivers/power/supply/maxim/max77729_charger.h"
#include "drivers/power/supply/nopmi_chg_common.h"
...
static int
nopmi_set_prop_system_temp_level(struct nopmi_chg *nopmi_chg,
				 const union power_supply_propval *val)
{
	int rc = 0;
	...
	if (nopmi_chg->system_temp_level ==
	    nopmi_chg->thermal_levels) { // thermal temp level
		rc = vote(nopmi_chg->usb_icl_votable, THERMAL_DAEMON_VOTER,
			  true, 0);
		if (rc < 0) {
	...
	if (nopmi_chg->system_temp_level == 0) {
		rc = vote(nopmi_chg->fcc_votable, THERMAL_DAEMON_VOTER, false,
			  0);
	} else {
		rc = vote(
			nopmi_chg->fcc_votable, THERMAL_DAEMON_VOTER, true,
			nopmi_chg->thermal_mitigation
					[nopmi_chg->system_temp_level] /
				1000); //divide 1000 to match maxim driver fcc as mA
	}
					...
					union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		rc = get_prop_battery_charging_enabled(
			nopmi_chg->jeita_ctl.usb_icl_votable, pval);
		break;
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		rc = set_prop_battery_charging_enabled(
			nopmi_chg->jeita_ctl.usb_icl_votable, val);
		break;
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_NOW:
		val->intval =
			get_effective_result(g_nopmi_chg->usb_icl_votable);
		ret = 0;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
			      ...
			      const union power_supply_propval *val)
{
	...
	pr_err("2021.09.21 wsy %s psp=%d,val->intval=%d", __func__, psp,
	       val->intval);
#if 1
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	case POWER_SUPPLY_PROP_MTBF_CUR:
		pr_err("%s psp=%d Set MTBF current val->intval=%d", __func__,
		       psp, val->intval);
...
static int nopmi_parse_dt_jeita(struct nopmi_chg *chg, struct device_node *np)
{
	...
	if (of_property_read_u32(np, "jeita_temp_above_t4_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_above_t4_cv =
			JEITA_TEMP_ABOVE_T4_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_t3_to_t4_cv =
			JEITA_TEMP_T3_TO_T4_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_t2_to_t3_cv =
			JEITA_TEMP_T2_TO_T3_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_t1p5_to_t2_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_t1p5_to_t2_cv =
			JEITA_TEMP_T1P5_TO_T2_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_t1_to_t1p5_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_t1_to_t1p5_cv =
			JEITA_TEMP_T1_TO_T1P5_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_t0_to_t1_cv =
			JEITA_TEMP_T0_TO_T1_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_tn1_to_t0_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_tn1_to_t0_cv =
			JEITA_TEMP_TN1_TO_T0_CV;
	}
	if (of_property_read_u32(np, "jeita_temp_below_t0_cv", &val) >= 0)
		...
		chg->jeita_ctl.dt.jeita_temp_below_t0_cv =
			JEITA_TEMP_BELOW_T0_CV;
	}
	...
	if (of_property_read_u32(np, "temp_t1p5_thres_plus_x_degree", &val) >=
	    0)
...
static void start_nopmi_chg_workfunc(void)
{
	...
	if (g_nopmi_chg) {
		...
		schedule_delayed_work(
			&g_nopmi_chg->cvstep_monitor_work,
			msecs_to_jiffies(
				NOPMI_CHG_CV_STEP_MONITOR_WORKFUNC_GAP));
	}
}
...
static void nopmi_cv_step_monitor_work(struct work_struct *work)
{
	...
	struct nopmi_chg *nopmi_chg =
		container_of(work, struct nopmi_chg, cvstep_monitor_work.work);
#endif
	...
	pr_err("fg_cc_cv_step_check: batt_volt:%d batt_curr:%d", batt_volt,
	       batt_curr);
	/*discharging*/
	...
	for (i = 0; i < step_table_max; i++) {
		if ((batt_volt >= (pcc_cv_step_config[i].volt_lim -
				   CV_BATT_VOLT_HYSTERESIS)) &&
	...
	if (!stepdown ||
	    finalFCC <= pcc_cv_step_config[step_table_max - 1].curr_lim)
		goto out;
...
static int nopmi_chg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&nopmi_chg->cvstep_monitor_work,
			  nopmi_cv_step_monitor_work);
