...
bool max77729_fg_fuelalert_init(struct max77729_fuelgauge_data *fuelgauge,
				int soc, int volt);
static void
max77729_fg_periodic_read_power(struct max77729_fuelgauge_data *fuelgauge);
...
static void max77729_fg_model_load(struct max77729_fuelgauge_data *fg)
{
	...
	if (max77729_read_word(fg->i2c, DESIGNCAP_REG) !=
	    fg->battery_data->Capacity) {
		force_update = true;
	}
	...
	if (max77729_read_word(fg->i2c, STATUS_REG) & BIT(1) || force_update) {
		...
		while (retry < 10 && i < FG_MODEL_DATA_COUNT) {
			...
			for (i = 0; i < FG_MODEL_DATA_COUNT; i++) {
				max77729_write_word(fg->i2c, 0x80 + i,
						    fg->fg_model_data[i]);
			}
			for (i = 0; i < FG_MODEL_DATA_COUNT; i++) {
				if (max77729_read_word(fg->i2c, 0x80 + i) !=
				    fg->fg_model_data[i])
					break;
			}
			retry++;
		...
		for (i = 0; i < fg->verify_selected_reg_length; i++) {
			max77729_write_word(fg->i2c, fg_verify[i].addr,
					    fg_verify[i].data);
			if (max77729_read_word(fg->i2c, fg_verify[i].addr) !=
			    fg_verify[i].data) {
				pr_err("%s: fg verified customer data error reg %x data %x \n",
				       __func__, fg_verify[i].addr,
				       fg_verify[i].data);
			}
		}
		...
		retry = 0;
		while (((max77729_read_word(fg->i2c, 0xBB) & 0x0020) ==
			0x0020) &&
		       retry < 10) {
			msleep(100);
			retry++;
...
re_calculation:
	...
	max77729_write_word(fuelgauge->i2c, FULLCAP_NOM_REG,
...
static void max77729_fg_periodic_read(struct max77729_fuelgauge_data *fuelgauge)
{
	...
	for (i = 0; i < 16; i++) {
		...
		for (reg = 0; reg < 0x10; reg++) {
			data[reg] = max77729_read_word(fuelgauge->i2c,
						       reg + i * 0x10);
			...
			/* verify fg reg */
			if (!fuelgauge->skip_fg_verify && fg_verify &&
			    v_cnt < fuelgauge->verify_selected_reg_length) {
				if (fg_verify[v_cnt].addr == reg + i * 0x10) {
					if (fg_verify[v_cnt].data !=
					    data[reg]) {
						pr_err("%s:[%d] addr(0x%x 0x%x) data(0x%x 0x%x)\n",
						       __func__, v_cnt,
						       fg_verify[v_cnt].addr,
						       reg + i * 0x10,
						       fg_verify[v_cnt].data,
						       data[reg]);
						err_cnt++;
					}
					v_cnt++;
...
static int
max77729_fg_check_battery_present(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 status_data[2];
	int ret = 1;

	if (max77729_bulk_read(fuelgauge->i2c, STATUS_REG, 2, status_data) <
	    0) {
				   ...
				   int vempty_mode)
{
	...
	case VEMPTY_MODE_SW:
		...
		if (max77729_bulk_write(fuelgauge->i2c, VALRT_THRESHOLD_REG, 2,
					valrt_data) < 0) {
	...
	default:
		...
		valrt_data[1] = 0xFF;
		valrt_data[0] =
			fuelgauge->battery_data->sw_v_empty_vol_cisd / 20;
		if (max77729_bulk_write(fuelgauge->i2c, VALRT_THRESHOLD_REG, 2,
					valrt_data) < 0) {
			pr_info("%s: Failed to write VALRT_THRESHOLD_REG\n",
				__func__);
...
static int max77729_fg_read_temp(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 data[4] = { 0, 0, 0, 0 };
	int temper = 0;
	static int tempRangeIdOld =
		0; // 3 ranges for QRtable: 1: >-5; 2: -5~-15; 3:<-15.
	int tempRangeIdNew = 0;

	if (max77729_fg_check_battery_present(fuelgauge)) {
		if (max77729_bulk_read(fuelgauge->i2c, TEMPERATURE_REG, 2,
				       data) < 0) {
	...
	if (fuelgauge->temperature > 0) {
		tempRangeIdNew = 1;
	} else if (fuelgauge->temperature <= 0 &&
		   fuelgauge->temperature > -100) {
		tempRangeIdNew = 2;
...
static int
max77729_fg_read_fullcaprep(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	int ret;
...
static int
max77729_fg_read_fullcapnom(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	int ret;
...
static int
max77729_fg_read_avg_current(struct max77729_fuelgauge_data *fuelgauge,
			     int unit)
{
	static int cnt;
	u8 data2[2];
				 ...
				 int unit)
{
	...
	if (fuelgauge->fg_resistor != 2 && i_current != 0) {
		inow_comp = (int)((fuelgauge->fg_resistor - 2) * 10 /
				     ...
				     int unit)
{
	...
	if (fuelgauge->fg_resistor != 2 && avg_current != 0) {
		avg_inow_comp = (int)((fuelgauge->fg_resistor - 2) * 10 /
				      fuelgauge->fg_resistor) *
				avg_inow / 10;
...
int max77729_fg_reset_soc(struct max77729_fuelgauge_data *fuelgauge)
{
	...
	pr_info("%s: Before quick-start - current(%d), avg current(%d)\n",
		...
		max77729_fg_read_avg_current(fuelgauge,
...
static int
max77729_fg_check_status_reg(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 status_data[2];
	int ret = 0;
	...
	status_data[1] = 0;
	if (max77729_bulk_write(fuelgauge->i2c, STATUS_REG, 2, status_data) <
	    0) {
				 ...
				 int data)
{
	...
	case FG_VOLTAGE:
		...
		break;
	case FG_CURRENT:
		ret = max77729_fg_read_current(fuelgauge,
					       SEC_BATTERY_CURRENT_MA);
		break;
	case FG_CURRENT_AVG:
		ret = max77729_fg_read_avg_current(fuelgauge,
						   SEC_BATTERY_CURRENT_MA);
		break;
	...
	case FG_ISYS:
		...
		break;
	case FG_ISYS_AVG:
		ret = max77729_fg_read_isys_avg(fuelgauge,
						SEC_BATTERY_CURRENT_MA);
		break;
	case FG_VSYS:
...
max77729_fg_periodic_read_power(struct max77729_fuelgauge_data *fuelgauge)
{
	int isys, isys_avg, vsys, iin, vbyp, qh, vfsoc, fullcaprep, vcell,
		avgvcell, fgcurrent, avgcurrent, remcap, mixcap, avcap, capnom,
		fullcap;
	u16 vempty, designcap, ichgterm, misccfg, fullsocthr, learncfg, ain0,
		tgain, toff, curve, qres, filtercfg, convgcfg;
	...
	pr_debug(
		"[FG periodic] ISYS(%dmA),ISYSAVG(%dmA),VSYS(%dmV),IIN(%dmA),VBYP(%dmV) QH(%d uah) VFSOC(%d) FULLCAPREP(%d) ICHGTERM(0x%x), MISC(0x%x), FULLSOCTHR(0x%x), VEMPTY(0x%x), DESIGNCAP(0x%x), LEARNCFG(0x%x) AIN0(0x%x) TGAIN(0x%x) TOFF(0x%x) CURVE(0x%x)",
		isys, isys_avg, vsys, iin, vbyp, qh, vfsoc, fullcaprep,
		ichgterm, misccfg, fullsocthr, vempty, designcap, learncfg,
		ain0, tgain, toff, curve);

	pr_debug(
		"[FG periodic] VCELL(%dmV) AVGVCELL(%dmV) CURRENT(%dmA) AVGCURRENT(%dmA) REPCAP(%d), QRES(0x%x), FILTERCFG(0x%x), CONVGCFG(0x%x), MIXCAP(%d), FULLCAP(%d), AVCAP(%d), CAPNOM(%d)",
		vcell, avgvcell, fgcurrent, avgcurrent, remcap, qres, filtercfg,
		convgcfg, mixcap, fullcap, avcap, capnom);
}

static void
max77729_fg_read_power_log(struct max77729_fuelgauge_data *fuelgauge)
{
	int vnow, inow;
...
int max77729_fg_alert_init(struct max77729_fuelgauge_data *fuelgauge, int soc,
			   int low_volt_thres)
{
	...
	/* Using RepSOC */
	if (max77729_bulk_read(fuelgauge->i2c, MISCCFG_REG, 2, misccgf_data) <
	    0) {
		pr_err("%s: Failed to read MISCCFG_REG\n", __func__);
		return -1;
	}
	...
	if (max77729_bulk_write(fuelgauge->i2c, MISCCFG_REG, 2, misccgf_data) <
	    0) {
		pr_info("%s: Failed to write MISCCFG_REG\n", __func__);
		return -1;
	}
	...
	salrt_data[1] = 0xff;
	salrt_data[0] = soc;
	if (max77729_bulk_write(fuelgauge->i2c, SALRT_THRESHOLD_REG, 2,
				salrt_data) < 0) {
	...
	talrt_data[1] = 0x7F;
	talrt_data[0] = 0x80;
	if (max77729_bulk_write(fuelgauge->i2c, TALRT_THRESHOLD_REG, 2,
				talrt_data) < 0) {
	...
	/* Enable SOC alerts */
	if (max77729_bulk_read(fuelgauge->i2c, CONFIG_REG, 2, config_data) <
	    0) {
	...
	if (max77729_bulk_write(fuelgauge->i2c, CONFIG_REG, 2, config_data) <
	    0) {
...
bool max77729_fg_init(struct max77729_fuelgauge_data *fuelgauge)
{
	...
	if (fuelgauge->pdata->thermal_source != SEC_BATTERY_THERMAL_SOURCE_FG) {
		if (max77729_bulk_read(fuelgauge->i2c, CONFIG_REG, 2, data) <
		    0) {
			pr_err("%s : Failed to read CONFIG_REG\n", __func__);
			return false;
		}
		...
		if (max77729_bulk_write(fuelgauge->i2c, CONFIG_REG, 2, data) <
		    0) {
			pr_info("%s : Failed to write CONFIG_REG\n", __func__);
			return false;
		}
...
static int
max77729_fg_check_capacity_max(struct max77729_fuelgauge_data *fuelgauge,
			       int capacity_max)
{
	int cap_max, cap_min;
	...
	return (capacity_max < cap_min) ?
		       cap_min :
		       ((capacity_max >= cap_max) ? cap_max : capacity_max);
}

static int
max77729_fg_get_soc_decimal(struct max77729_fuelgauge_data *fuelgauge)
{
	int raw_soc;
...
static int
max77729_fg_get_soc_decimal_rate(struct max77729_fuelgauge_data *fuelgauge)
{
	int soc, i;
...
static int
max77729_fg_calculate_dynamic_scale(struct max77729_fuelgauge_data *fuelgauge,
				    int capacity, bool scale_by_full)
{
	...
	raw_soc_val.intval =
		max77729_get_fuelgauge_value(fuelgauge, FG_RAW_SOC) / 10;
...
static void
max77729_lost_soc_check_trigger_cond(struct max77729_fuelgauge_data *fuelgauge,
				     int raw_soc, int d_raw_soc, int d_remcap,
				     int d_qh)
{
	...
	if (d_raw_soc >= fuelgauge->lost_soc.trig_d_soc &&
		...
		if (d_raw_soc >= fuelgauge->lost_soc.guarantee_soc)
			fuelgauge->lost_soc.weight +=
				d_raw_soc / fuelgauge->lost_soc.guarantee_soc;
		else
				      ...
				      int request_soc, int d_qh, int d_remcap)
{
	...
	fullcaprep = max77729_fg_read_fullcaprep(fuelgauge);
	if (fullcaprep < 0) {
		fullcaprep = fuelgauge->battery_data->Capacity *
	...
	if ((vavg < fuelgauge->lost_soc.min_vol) && (vavg > 0) &&
	    (gap_cap < onecap)) {
		gap_cap = onecap; /* reduce 1% */
		pr_info("%s: ing: vavg(%d) is under min_vol(%d), reduce cap more(%d)\n",
			__func__, vavg, fuelgauge->lost_soc.min_vol,
			(fullcaprep / 100));
	}
	...
	if (fuelgauge->lost_soc.lost_cap > 0) {
		...
		pr_info("%s: ing: (unit:0.1%%) calc_soc(%d), lost_soc(%d), lost_cap(%d), d_qh(%d), weight(%d)\n",
			__func__, request_soc + lost_soc, lost_soc,
			fuelgauge->lost_soc.lost_cap, d_qh,
			fuelgauge->lost_soc.weight);
	} else {
		lost_soc = 0;
		max77729_lost_soc_reset(fuelgauge);
		pr_info("%s: done: (unit:0.1%%) request_soc(%d), lost_soc(%d), lost_cap(%d)\n",
			__func__, request_soc, lost_soc,
			fuelgauge->lost_soc.lost_cap);
	}
				 ...
				 int request_soc)
{
	...
	max77729_lost_soc_check_trigger_cond(fuelgauge, raw_soc, d_raw_soc,
					     d_remcap, d_qh);
	...
	report_soc =
		request_soc + max77729_lost_soc_calc_soc(fuelgauge, request_soc,
							 d_qh, d_remcap);
	...
	struct max77729_fuelgauge_data *fuelgauge)
{
	...
	if (max77729_bulk_read(fuelgauge->i2c, FG_INIT_RESULT_REG, 2, data) <
	    0) {
	...
	case FG_DATA:
		for (j = 0; j <= 0x0F; j++) {
			for (addr = 0; addr < 0x10; addr++) {
				data = max77729_read_word(fuelgauge->i2c,
							  addr + j * 0x10);
	...
	case FG_DATA:
		if (sscanf(buf, "0x%8x 0x%8x", &x, &y) == 2) {
			if (x >= VALRT_THRESHOLD_REG && x <= VFSOC_REG) {
				...
				if (max77729_write_word(fuelgauge->i2c, addr,
							data) < 0)
				    ...
				    union power_supply_propval *val)
{
	...
	union power_supply_propval usb_online_val = {
		0,
	};
	u8 data[2] = { 0, 0 };
	int vbat_mv;
	...
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		if (fuelgauge->battery_data->battery_id ==
		    BATTERY_VENDOR_UNKNOWN) {
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval =
			max77729_get_fuelgauge_value(fuelgauge, FG_VOLTAGE);
		val->intval *= 1000;
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = max77729_fg_read_current(
			fuelgauge,
			SEC_BATTERY_CURRENT_UA); //match onpmi & factory test cit apk
		val->intval =
			val->intval; //fix xts issue the current shouled be positive when charging
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		switch (val->intval) {
		case SEC_BATTERY_CURRENT_UA:
			val->intval = max77729_fg_read_avg_current(
				fuelgauge, SEC_BATTERY_CURRENT_UA);
			break;
		case SEC_BATTERY_CURRENT_MA:
		default:
			fuelgauge->current_avg = val->intval =
				max77729_get_fuelgauge_value(fuelgauge,
							     FG_CURRENT_AVG);
			break;
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		val->intval =
			max77729_get_fuelgauge_value(fuelgauge, FG_FULLCAP);
		break;
		/* Full Capacity */
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		switch (val->intval) {
		case SEC_BATTERY_CAPACITY_DESIGNED:
			val->intval = max77729_get_fuelgauge_value(fuelgauge,
								   FG_FULLCAP);
			break;
		...
		case SEC_BATTERY_CAPACITY_AGEDCELL:
			val->intval = max77729_get_fuelgauge_value(
				fuelgauge, FG_FULLCAPNOM);
			break;
		...
		case SEC_BATTERY_CAPACITY_FULL:
			val->intval = max77729_get_fuelgauge_value(
				fuelgauge, FG_FULLCAPREP);
			break;
		case SEC_BATTERY_CAPACITY_QH:
			val->intval =
				max77729_get_fuelgauge_value(fuelgauge, FG_QH);
			break;
		case SEC_BATTERY_CAPACITY_VFSOC:
			val->intval = max77729_get_fuelgauge_value(
				fuelgauge, FG_QH_VF_SOC);
			break;
		}
		break;
		/* SOC (%) */
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		} else if ((true == maxim_battery_full) &&
			   (val->intval >= 995 && val->intval <= 1000)) {
			val->intval = 1000;
		} else {
		...
		if (fuelgauge->pdata->capacity_calculation_type &
			...
			val->intval = max77729_lost_soc_get(
				fuelgauge, fuelgauge->raw_capacity);

		val->intval /= 10;

		if (fuelgauge->is_fuel_alerted) {
			if (fuelgauge->info.soc <= 100)
				max77729_fg_fuelalert_init(
					fuelgauge,
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL: {
		int fullcap =
			max77729_get_fuelgauge_value(fuelgauge, FG_FULLCAPNOM);
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_ERROR_CAUSE:
			val->intval = max77729_fg_check_initialization_result(
				fuelgauge);
			break;
		case POWER_SUPPLY_EXT_PROP_MEASURE_SYS:
			...
			case SEC_BATTERY_ISYS_AVG_UA:
				val->intval = max77729_fg_read_isys_avg(
					fuelgauge, SEC_BATTERY_CURRENT_UA);
				break;
			case SEC_BATTERY_ISYS_AVG_MA:
				val->intval = max77729_fg_read_isys_avg(
					fuelgauge, SEC_BATTERY_CURRENT_MA);
				break;
			case SEC_BATTERY_ISYS_UA:
				val->intval = max77729_fg_read_isys(
					fuelgauge, SEC_BATTERY_CURRENT_UA);
				break;
		...
		case POWER_SUPPLY_EXT_PROP_MEASURE_INPUT:
			...
			case SEC_BATTERY_IIN_UA:
				val->intval = max77729_fg_read_iin(
					fuelgauge, SEC_BATTERY_CURRENT_UA);
				break;
		...
		case POWER_SUPPLY_EXT_PROP_JIG_GPIO:
			if (fuelgauge->pdata->jig_gpio)
				val->intval = gpio_get_value(
					fuelgauge->pdata->jig_gpio);
			else
				val->intval = -1;
			/* pr_info("%s: jig gpio = %d \n", __func__, val->intval); */
			break;
		case POWER_SUPPLY_EXT_PROP_FILTER_CFG:
			max77729_bulk_read(fuelgauge->i2c, FILTER_CFG_REG, 2,
					   data);
...
static int
max77729_fuelgauge_parse_dt(struct max77729_fuelgauge_data *fuelgauge);
#endif
				    ...
				    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (fuelgauge->pdata->capacity_calculation_type &
			...
			max77729_fg_calculate_dynamic_scale(fuelgauge,
							    val->intval, true);
		break;
#if defined(CONFIG_EN_OOPS)
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (!is_nocharge_type(fuelgauge->cable_type)) {
			...
			max77729_fg_fuelalert_init(
				fuelgauge, fuelgauge->pdata->fuel_alert_soc,
				2800);
			/* } */
		}
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (true) {
			...
			if (val->intval < 0) {
				reg_data = max77729_read_word(fuelgauge->i2c,
							      DESIGNCAP_REG);
				if (reg_data ==
				    fuelgauge->battery_data->Capacity) {
					max77729_write_word(
						fuelgauge->i2c, DESIGNCAP_REG,
			...
			if (val->intval < 0 && !low_temp_wa) {
				low_temp_wa = true;
				max77729_write_word(fuelgauge->i2c, 0x29,
						    0xCEA7);
				pr_info("%s: FilterCFG(0x%0x)\n", __func__,
					max77729_read_word(fuelgauge->i2c,
							   0x29));
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL: {
		...
		if (val->intval > fuelgauge->pdata->full_condition_soc ||
		    val->intval <=
			    (fuelgauge->pdata->full_condition_soc - 10)) {
		...
		if (max77729_write_word(fuelgauge->i2c, FULLSOCTHR_REG,
			...
			reg_fullsocthr = max77729_read_word(fuelgauge->i2c,
							    FULLSOCTHR_REG);
			pr_info("%s: FullSOCThr %d%%(0x%04X)\n", __func__,
				val_soc, reg_fullsocthr);
		}
	} break;
#endif
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_FILTER_CFG:
			...
			max77729_bulk_write(fuelgauge->i2c, FILTER_CFG_REG, 2,
					    data);
...
static u8 fgauge_get_battery_id(void)
{
	...
	if (max_verify_psy != NULL) {
		rc = power_supply_get_property(
			max_verify_psy, POWER_SUPPLY_PROP_CHIP_OK, &pval);
		if (rc < 0)
			pr_err("fgauge_get_profile_id: get romid error.\n");

		if (pval.intval) {
			rc = power_supply_get_property(
				max_verify_psy, POWER_SUPPLY_PROP_PAGE0_DATA,
				&pval);
			if (rc < 0) {
...
{
	struct max77729_fuelgauge_data *fuelgauge = container_of(
		work, struct max77729_fuelgauge_data, isr_work.work);
...
{
	struct max77729_fuelgauge_data *fuelgauge = container_of(
		work, struct max77729_fuelgauge_data, shutdown_delay_work.work);
	...
	pr_info("%s: wzt bms set battery prop now: %d \n", __func__,
		shutdown_val.intval);
	psy_do_property("battery", set, POWER_SUPPLY_PROP_SHUTDOWN_DELAY,
			shutdown_val);
}
...
static void
max77729_fuelgauge_parse_dt_lost_soc(struct max77729_fuelgauge_data *fuelgauge,
				     struct device_node *np)
{
	...
	pr_info("%s: trigger soc(%d), d_soc(%d), scale(%d), guarantee_soc(%d), min_vol(%d)\n",
		__func__, fuelgauge->lost_soc.trig_soc,
		fuelgauge->lost_soc.trig_d_soc, fuelgauge->lost_soc.trig_scale,
		fuelgauge->lost_soc.guarantee_soc, fuelgauge->lost_soc.min_vol);
}
...
{
	struct device_node *np =
		of_find_node_by_name(NULL, "max77729-fuelgauge");
	...
	if (np == NULL) {
		...
		ret = of_property_read_u32(
			np, "fuelgauge,capacity_calculation_type",
		...
		pdata->repeated_fuelalert = of_property_read_bool(
			np, "fuelgauge,repeated_fuelalert");

		fuelgauge->using_temp_compensation = of_property_read_bool(
			np, "fuelgauge,using_temp_compensation");
		...
		if (fuelgauge->using_temp_compensation) {
			...
			ret = of_property_read_u32(
				np, "fuelgauge,vempty_recover_time",
		...
		fuelgauge->using_hw_vempty =
			of_property_read_bool(np, "fuelgauge,using_hw_vempty");
		if (fuelgauge->using_hw_vempty) {
			ret = of_property_read_u32(
				np, "fuelgauge,sw_v_empty_voltage",
			...
			ret = of_property_read_u32(
				np, "fuelgauge,sw_v_empty_voltage_cisd",
			...
			ret = of_property_read_u32(
				np, "fuelgauge,sw_v_empty_recover_voltage",
				&fuelgauge->battery_data
					 ->sw_v_empty_recover_vol);
			if (ret < 0)
		...
		if (pdata->jig_gpio >= 0) {
			...
			pdata->jig_low_active = of_property_read_bool(
				np, "fuelgauge,jig_low_active");
		} else {
		...
		if (p) {
			...
			fuelgauge->dec_rate_len =
				len / sizeof(*fuelgauge->dec_rate_seq);
			//pr_err("%s: len= %ld, length= %d, %d\n", __func__,
			//		sizeof(int) * len, len, fuelgauge->verify_selected_reg_length);
			ret = of_property_read_u32_array(
				np, "fuelgauge,soc_decimal_rate",
				fuelgauge->dec_rate_seq,
				fuelgauge->dec_rate_len);
			if (ret) {
		...
		pdata->bat_id_gpio =
			of_get_named_gpio(np, "fuelgauge,bat_id_gpio", 0);
		...
		fuelgauge->shutdown_delay_enable = of_property_read_bool(
			np, "battery,shutdown-delay-enable");
		...
		if (np == NULL) {
			...
			if (len != FG_RESET_DATA_COUNT) {
				...
				fuelgauge->fg_reset_data = kzalloc(
					sizeof(struct fg_reset_wa), GFP_KERNEL);
				ret = of_property_read_u32_array(
					np, prop_name,
					(u32 *)fuelgauge->fg_reset_data,
					FG_RESET_DATA_COUNT);
				if (ret < 0) {
			...
			if (len != FG_MODEL_DATA_COUNT) {
				...
				fuelgauge->fg_model_data = kzalloc(
					FG_MODEL_DATA_COUNT * sizeof(u32),
					GFP_KERNEL);
				ret = of_property_read_u32_array(
					np, prop_name, fuelgauge->fg_model_data,
					FG_MODEL_DATA_COUNT);
				if (ret < 0) {
			...
			if (p) {
				fuelgauge->verify_selected_reg =
					kzalloc(len, GFP_KERNEL);
				fuelgauge->verify_selected_reg_length =
					len / (int)sizeof(struct verify_reg);
				/* pr_err("%s: len= %ld, length= %d, %d\n", __func__, */
				/* sizeof(int) * len, len, fuelgauge->verify_selected_reg_length); */
				ret = of_property_read_u32_array(
					np, prop_name,
					(u32 *)fuelgauge->verify_selected_reg,
					len / sizeof(u32));
				if (ret) {
...
static int max77729_fuelgauge_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_BATT_VERIFY_BY_DS28E16
	if (max77729_read_word(max77729->fuelgauge, STATUS_REG) &
	    BIT(1)) { //if fg chip por flag set, should be wait for battery id from ds28e16
		max_verify_psy = power_supply_get_by_name("batt_verify");
		if (!max_verify_psy) {
	...
	fuelgauge_data =
		kzalloc(sizeof(max77729_fuelgauge_platform_data_t), GFP_KERNEL);
	if (!fuelgauge_data) {
		ret = -ENOMEM;
		goto err_free;
	}
	...
	fuelgauge->battery_data =
		kzalloc(sizeof(struct battery_data_t), GFP_KERNEL);
