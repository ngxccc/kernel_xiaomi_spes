...
extern int
set_prop_battery_charging_enabled(struct votable *usb_icl_votable,
...
extern void
max77729_rerun_chgdet(struct max77729_usbc_platform_data *usbpd_data);
...
static enum power_supply_property max77729_usb_props[] = {
	...
	POWER_SUPPLY_PROP_PD_ACTIVE, POWER_SUPPLY_PROP_MTBF_CUR,
#if 0
	POWER_SUPPLY_PROP_VOLTAGE_MAX,
...
static void max77729_chg_set_mode_state(struct max77729_charger_data *charger,
					unsigned int state);
static void
max77729_set_switching_frequency(struct max77729_charger_data *charger,
				 int frequency);
					 ...
					 int pd_active)
{
	...
	if (charger->psy_bms) {
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_CHIP_OK, &prop);
		if (rc < 0) {
		...
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_CAPACITY, &prop);
		if (rc < 0) {
	...
	rc = psy_do_property("bms", set, POWER_SUPPLY_PROP_FASTCHARGE_MODE,
			     prop);
	if (rc < 0) {
...
static bool max77729_charger_unlock(struct max77729_charger_data *charger)
{
	...
	do {
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_CNFG_06,
				  &reg_data);
		chgprot = ((reg_data & 0x0C) >> 2);
		if (chgprot != 0x03) {
			max77729_update_reg(charger->i2c,
					    MAX77729_CHG_REG_CNFG_06,
					    (0x03 << 2), (0x03 << 2));
			need_init = true;
			msleep(20);
		} else {
				     ...
				     bool enable)
{
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_00,
			    (enable ? CHG_CNFG_00_WDTEN_MASK : 0),
				      ...
				      int float_voltage)
{
	...
	ret = max77729_read_reg(charger->i2c, MAX77729_CHG_REG_CNFG_04,
				&reg_data);
...
static int max77729_get_charging_health(struct max77729_charger_data *charger)
{
	...
	if (charger->is_charging) {
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_00,
				  &reg_data);
	...
	psy_do_property("bms", get, POWER_SUPPLY_EXT_PROP_MEASURE_INPUT,
			val_iin);
	...
	if (vbus_state == 0x02) { /*  CHGIN_OVLO */
		...
		if (abnormal_status || (vbus_state == 0x00) ||
		    (vbus_state == 0x01))
...
static int
max77729_get_input_current_type(struct max77729_charger_data *charger,
				int cable_type)
{
	u8 reg_data;
	int get_current = 0;
	...
	if (cable_type == SEC_BATTERY_CABLE_WIRELESS) {
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_CNFG_10,
				  &reg_data);
		...
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_CNFG_09,
				  &reg_data);
...
{
	if (is_wireless_type(charger->cable_type))
		return max77729_get_input_current_type(
			charger, SEC_BATTERY_CABLE_WIRELESS);
	else
		return max77729_get_input_current_type(charger,
						       SEC_BATTERY_CABLE_TA);
}

static void reduce_input_current(struct max77729_charger_data *charger,
				 int curr)
{
	...
	input_current -= curr;
	input_current = (input_current > max_value) ?
				max_value :
				((input_current < MINIMUM_INPUT_CURRENT) ?
					 MINIMUM_INPUT_CURRENT :
					 input_current);
	...
	vote(charger->usb_icl_votable, MAIN_CHG_AICL_VOTER, true,
	     input_current);
}
...
static void max77729_check_cnfg12_reg(struct max77729_charger_data *charger)
{
	...
	if (is_valid) {
		...
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_CNFG_12,
				  &reg_data);
					...
					int path)
{
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_12, cnfg12,
				   ...
				   int enable)
{
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_07, cnfg07,
					...
					int enable)
{
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_03, cnfg03,
			    CHG_CNFG_03_REG_AUTO_SHIPMODE_MASK);
...
static void
max77729_set_wireless_input_current(struct max77729_charger_data *charger,
				    int input_current)
{
	...
	if (is_wireless_type(charger->cable_type)) {
		...
		if (charger->wc_current > charger->wc_pre_current)
			max77729_set_charge_current(charger,
...
static int
max77729_check_wcin_before_otg_on(struct max77729_charger_data *charger)
{
	union power_supply_propval value = {
		0,
	};
	...
	max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_00, &reg_data);
	reg_data =
		((reg_data & MAX77729_WCIN_DTLS) >> MAX77729_WCIN_DTLS_SHIFT);
	if ((reg_data != 0x03) ||
	    (charger->pdata->wireless_charger_name == NULL))
		return 0;
...
static int max77729_set_otg(struct max77729_charger_data *charger, int enable)
{
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_02,
			    MAX77729_OTG_ILIM_1500
				    << CHG_CNFG_02_OTG_ILIM_SHIFT,
	...
	max77729_read_reg(charger->i2c, MAX77729_CHG_REG_INT_MASK,
			  &chg_int_state);
					 ...
					 int input_current)
{
	...
	if (input_current <= SLOW_CHARGING_CURRENT_STANDARD &&
		...
		psy_do_property("battery", set, POWER_SUPPLY_PROP_CHARGE_TYPE,
				value);
	} else {
...
static void max77729_charger_initialize(struct max77729_charger_data *charger)
{
	...
	max77729_write_reg(charger->i2c, MAX77729_CHG_REG_INT_MASK,
			   0xff); //disable all chg interrupt
	...
	reg_data = (0x03 << 2);
	reg_data |= 0x60;
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_06, reg_data,
			    reg_data);
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_01,
			    MAX77729_RECYCLE_EN_ENABLE
				    << CHG_CNFG_01_RECYCLE_EN_SHIFT,
			    CHG_CNFG_01_RECYCLE_EN_MASK);

	/* OTG off(UNO on), boost off */
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_00, 0,
			    CHG_CNFG_00_OTG_CTRL);
	...
	/* UNO ILIM 1.0A */
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_05,
			    MAX77729_UNOILIM_1000
				    << CHG_CNFG_05_REG_UNOILIM_SHIFT,
			    CHG_CNFG_05_REG_UNOILIM_MASK);

	/* BAT to SYS OCP */
	max77729_set_b2sovrc(charger, charger->pdata->chg_ocp_current,
	...
	max77729_update_reg(charger->pmic_i2c, MAX77729_PMIC_REG_MAINCTRL1,
			    0x01, 0x01);
	...
	/* VBYPSET=5.0V */
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_11, 0x00,
			    CHG_CNFG_11_VBYPSET_MASK);
...
static void max77729_set_sysovlo(struct max77729_charger_data *charger,
				 int enable)
{
	u8 reg_data;

	max77729_read_reg(charger->pmic_i2c, MAX77729_PMIC_REG_SYSTEM_INT_MASK,
			  &reg_data);

	reg_data = enable ? reg_data & 0xDF : reg_data | 0x20;
	max77729_write_reg(charger->pmic_i2c, MAX77729_PMIC_REG_SYSTEM_INT_MASK,
			   reg_data);
...
static void max77729_chg_monitor_work(struct max77729_charger_data *charger)
{
	...
	reg_b2sovrc = (reg_b2sovrc & CHG_CNFG_05_REG_B2SOVRC_MASK) >>
		      CHG_CNFG_05_REG_B2SOVRC_SHIFT;
				...
				struct device_attribute *attr, char *buf)
{
	...
	switch (offset) {
	case CHIP_ID:
		max77729_read_reg(charger->pmic_i2c, MAX77729_PMIC_REG_PMICID1,
				  &data);
	...
	case DATA:
		if (sscanf(buf, "0x%8x 0x%8x", &x, &y) == 2) {
			if (x >= 0xB1 && x <= 0xC3) {
				...
				if (max77729_write_reg(charger->i2c, addr,
						       data) < 0)
...
ssize_t sec_otg_show_attrs(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
...
static void max77729_set_uno(struct max77729_charger_data *charger, int en)
{
	...
	pr_info("%s: UNO(%d), INT_MASK(0x%x), CHG_CNFG_00(0x%x)\n", __func__,
		charger->uno_on, chg_int_state, reg);
}

static void max77729_set_uno_iout(struct max77729_charger_data *charger,
				  int iout)
{
	u8 reg = 0;
...
static void max77729_set_uno_vout(struct max77729_charger_data *charger,
				  int vout)
{
	u8 reg = 0;
	...
	max77729_update_reg(charger->i2c, MAX77729_CHG_REG_CNFG_11, reg,
			    CHG_CNFG_11_VBYPSET_MASK);
...
#if !defined(CONFIG_NOPMI_CHARGER)
static int
get_prop_battery_charging_enabled(struct max77729_charger_data *charger,
				  union power_supply_propval *val)
{
...
static int
set_prop_battery_charging_enabled(struct max77729_charger_data *charger,
				  const union power_supply_propval *val)
{
	int icl = MAIN_ICL_MIN;
	...
	if (val->intval == 0) {
		vote(charger->usb_icl_votable, MAIN_CHG_SUSPEND_VOTER, true,
		     icl);
	} else {
		vote(charger->usb_icl_votable, MAIN_CHG_SUSPEND_VOTER, false,
		     0);
	}
				     ...
				     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (!max77729_read_reg(charger->i2c, MAX77729_CHG_REG_INT_OK,
				       &reg_data)) {
		...
		if (charger->psy_bms) {
			rc = power_supply_get_property(
				charger->psy_bms, POWER_SUPPLY_PROP_VOLTAGE_AVG,
				&value);
			if (rc < 0) {
				value.intval = 3800;
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		val->intval = max77729_get_input_current_type(
			charger, (is_wireless_type(val->intval)) ?
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_01,
				  &reg_data);
		reg_data &= 0x0F;
		switch (reg_data) {
		case 0x01:
	...
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		val->intval =
			(charger->charge_mode == SEC_BAT_CHG_MODE_CHARGING ? 1 :
									     0);
		break;
					...
					unsigned int state)
{
	...
	case MAX77729_MODE_4_BUCK_ON:
		if (state == SEC_BAT_CHG_MODE_CHARGING)
			...
			charger->cnfg00_mode =
				MAX77729_MODE_E_BUCK_BOOST_OTG_ON;
		else if (state == SEC_BAT_CHG_MODE_UNO_ON)
			charger->cnfg00_mode =
				MAX77729_MODE_C_BUCK_BOOST_UNO_ON;
		break;
		/* buck, charger on */
	case MAX77729_MODE_5_BUCK_CHG_ON:
		if (state == SEC_BAT_CHG_MODE_BUCK_OFF)
			...
			charger->cnfg00_mode =
				MAX77729_MODE_F_BUCK_CHG_BOOST_OTG_ON;
		else if (state == SEC_BAT_CHG_MODE_UNO_ON)
			charger->cnfg00_mode =
				MAX77729_MODE_D_BUCK_CHG_BOOST_UNO_ON;
		break;
	case MAX77729_MODE_8_BOOST_UNO_ON:
		...
		/* UNO -> OTG */
			...
			max77729_update_reg(charger->i2c,
					    MAX77729_CHG_REG_CNFG_00,
					    MAX77729_MODE_4_BUCK_ON,
					    CHG_CNFG_00_MODE_MASK);
				     ...
				     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (false) { // (is_nocharge_type(charger->cable_type)) {
			...
			//modify by HTH-234718 at 2022/05/26 begin
			max77729_update_reg(charger->i2c,
					    MAX77729_CHG_REG_CNFG_12, 0x00,
					    CHG_CNFG_12_VCHGIN_REG_MASK);
			//modify by HTH-234718 at 2022/05/26 end
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX: {
		...
		if (is_wireless_type(charger->cable_type))
			max77729_set_wireless_input_current(charger,
							    input_current);
		else
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		...
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_INT_OK,
				  &reg_data);
	...
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		...
		vote(charger->chgctrl_votable, "charger-enable", true,
		     charger->charge_mode);
		break;
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHGINSEL:
			max77729_change_charge_path(charger,
						    charger->cable_type);
			break;
		...
		case POWER_SUPPLY_EXT_PROP_FGSRC_SWITCHING: {
			...
			max77729_update_reg(
				charger->i2c, MAX77729_CHG_REG_CNFG_07,
			...
			pr_info("%s: POWER_SUPPLY_EXT_PROP_FGSRC_SWITCHING(%d): reg(0x%x) val(0x%x)\n",
				__func__, reg_fgsrc, MAX77729_CHG_REG_CNFG_07,
				reg_data);
		} break;
		case POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED:
				     ...
				     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if (val->intval) {
			...
			max77729_update_reg(
				charger->i2c, MAX77729_CHG_REG_CNFG_02,
				MAX77729_OTG_ILIM_1500
					<< CHG_CNFG_02_OTG_ILIM_SHIFT,
				CHG_CNFG_02_OTG_ILIM_MASK);
		} else {
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		if (!charger->psy_bms)
			charger->psy_bms = power_supply_get_by_name("bms");
		if (charger->psy_bms)
			rc = power_supply_get_property(charger->psy_bms, psp,
						       pval);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		pval->intval =
			!(get_client_vote_locked(charger->usb_icl_votable,
						 MAIN_CHG_ENABLE_VOTER) ==
			  MAIN_ICL_MIN);
		break;
	case POWER_SUPPLY_PROP_HEALTH:
			       ...
			       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		max77729_charger_unlock(charger);
		vote(charger->usb_icl_votable, MAIN_CHG_ENABLE_VOTER,
		     !val->intval, MAIN_ICL_MIN);
		vote(charger->mainfcc_votable, MAIN_CHG_ENABLE_VOTER,
		     !val->intval, 200);
		if (val->intval) {
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		pr_err("%s: Set input suspend prop, value:%d\n", __func__,
		       val->intval);
		vote(charger->usb_icl_votable, MAIN_CHG_SUSPEND_VOTER,
		     !!val->intval, 0);
		if (val->intval) {
			vote(charger->chgctrl_votable, MAIN_CHG_SUSPEND_VOTER,
			     true, SEC_BAT_CHG_MODE_BUCK_OFF);
			/* max77729_chg_set_mode_state(charger, SEC_BAT_CHG_MODE_BUCK_OFF); */
		} else {
			vote(charger->chgctrl_votable, MAIN_CHG_SUSPEND_VOTER,
			     false, SEC_BAT_CHG_MODE_BUCK_OFF);
			/* max77729_chg_set_mode_state(charger, SEC_BAT_CHG_MODE_CHARGING); */
			//modify by HTH-234718 at 2022/05/26 begin
			vote(charger->usb_icl_votable, MAIN_CHG_AICL_VOTER,
			     false, 0); //cancel  aicl when buck open agoin
			//modify by HTH-234718 at 2022/05/26 end
		}
		/* power_supply_changed(charger->psy_batt); */
		break;
...
static irqreturn_t max77729_batp_irq(int irq, void *data)
{
	...
	if (!(reg_data & MAX77729_BATP_OK))
		psy_do_property("battery", set, POWER_SUPPLY_PROP_PRESENT,
				value);
...
static irqreturn_t max77729_bat_irq(int irq, void *data)
{
	...
	if (!(reg_int_ok & MAX77729_BAT_OK)) {
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_01,
				  &reg_data);
		reg_data = ((reg_data & MAX77729_BAT_DTLS) >>
			    MAX77729_BAT_DTLS_SHIFT);
		if (reg_data == 0x06) {
			pr_info("OCP(B2SOVRC)\n");
		...
		if (charger->otg_on) {
			...
			if (o_notify)
				send_otg_notify(o_notify,
						NOTIFY_EVENT_OVERCURRENT, 0);
#endif
			val.intval = 0;
			psy_do_property("otg", set, POWER_SUPPLY_PROP_ONLINE,
					val);
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, aicl_work.work);
	/* union power_supply_propval value; */
	...
	if (!charger->irq_aicl_enabled ||
	    charger->real_type == POWER_SUPPLY_TYPE_USB_HVDCP) { //||
	...
	if (!(aicl_state & MAX77729_AICL_OK)) {
		...
		if (is_not_wireless_type(charger->cable_type))
			max77729_check_slow_charging(charger,
...
static void max77729_enable_aicl_irq(struct max77729_charger_data *charger)
{
	...
	ret = request_threaded_irq(charger->irq_aicl, NULL, max77729_aicl_irq,
				   0, "aicl-irq", charger);
	if (ret < 0) {
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, redet_work.work);

	if (!charger->otg_on) {
		if (charger->real_type == POWER_SUPPLY_TYPE_USB_FLOAT ||
		    charger->real_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			max77729_rerun_chgdet(
				g_usbc_data); //trigger the next detecion routine
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, chgin_work.work);
	union power_supply_propval value = {
		0,
	};
	...
	max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_00,
			  &chgin_dtls);
	chgin_dtls = ((chgin_dtls & MAX77729_CHGIN_DTLS) >>
		      MAX77729_CHGIN_DTLS_SHIFT);
	...
	if (charger->psy_usb) {
		max77729_read_reg(charger->muic, REG_BC_STATUS,
				  &g_usbc_data->muic_data->bc_status);
		g_usbc_data->muic_data->vbusdet =
			((g_usbc_data->muic_data->bc_status & BIT_VBUSDet) ==
			 BIT_VBUSDet) ?
				1 :
				0;
		...
		if (g_usbc_data->muic_data->vbusdet) {
			...
			vote(charger->usb_icl_votable, MAX77729_CHG_VOTER, true,
			     0);
			vote(charger->fcc_votable, MAX77729_CHG_VOTER, true, 0);
			vote(charger->chgctrl_votable, "charger-enable", false,
			     charger->charge_mode);
			vote(charger->chgctrl_votable, MAIN_CHG_ENABLE_VOTER,
			     false, charger->charge_mode);
			vote(charger->chgctrl_votable, "JEITA_CHG_VOTER", false,
			     0);
			charger->batt_notify_count = 0;

			charger->real_type = POWER_SUPPLY_TYPE_UNKNOWN;
			value.intval = POWER_SUPPLY_TYPE_UNKNOWN;
			psy_do_property("usb", set, POWER_SUPPLY_PROP_REAL_TYPE,
					value);

			value.intval = SEC_BATTERY_CABLE_NONE;
			psy_do_property("bbc", set, POWER_SUPPLY_PROP_ONLINE,
					value);
			psy_do_property("bms", set, POWER_SUPPLY_PROP_ONLINE,
					value);
			value.intval = 0;
			psy_do_property("usb", set, POWER_SUPPLY_PROP_ONLINE,
					value);
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, batt_notify_work.work);
	union power_supply_propval pval = {
		0,
	};
	int usb_online = 0;
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, notify_work.work);
	union power_supply_propval pval = {
		0,
	};
	...
	if (charger->psy_usb) {
		...
		case POWER_SUPPLY_TYPE_USB:
			...
			if (g_usbc_data && g_usbc_data->muic_data) {
				...
				} else if (g_usbc_data->muic_data->pr_chg_type ==
					   2) {
					charger->real_type =
						POWER_SUPPLY_TYPE_USB_DCP;
				}
		...
		case POWER_SUPPLY_TYPE_USB_FLOAT:
			input_curr_limit =
				1000; //only for factory test,it shouble be 1000
			break;
		...
		if (charger->pd_active == 1) {
			...
			if (g_usbc_data->pd_data->pdo_list) {
				if (g_usbc_data->pd_data->pd_noti.sink_status
					    .available_pdo_num > 0) {
					if (g_usbc_data->pd_data->pd_noti
						    .sink_status
						    .available_pdo_num > 1) {
						input_curr_limit =
							g_usbc_data->pd_data
			...
			input_curr_limit = 2000;
			for (i = 0; i < g_usbc_data->pd_data->pd_noti
						.sink_status.available_pdo_num;
			     ++i) {
				pPower_list =
					&g_usbc_data->pd_data->pd_noti
						 .sink_status.power_list[i + 1];
				if (pPower_list->apdo &&
				    pPower_list->max_voltage > 10000 &&
				    g_usbc_data->pd_data->pd_noti.sink_status
						    .selected_pdo_num == 0) {
					if (pPower_list->max_current > 2000) {
						input_curr_limit =
							pPower_list->max_current;
						max77729_select_pps(
							i + 1, 6000,
							pPower_list
								->max_current);
		...
		vote(charger->usb_icl_votable, MAIN_CHG_AICL_VOTER, true,
		     input_curr_limit);
		vote(charger->fcc_votable, MAX77729_CHG_VOTER, true,
		     fastchg_curr);
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, adapter_change_work.work);
	union power_supply_propval value = {
		0,
	};
	union power_supply_propval pvalue = {
		0,
	};
	...
	max77729_read_reg(charger->muic, REG_CC_STATUS0,
			  &g_usbc_data->cc_data->cc_status0);
	max77729_read_reg(charger->muic, REG_BC_STATUS,
			  &g_usbc_data->muic_data->bc_status);
	g_usbc_data->muic_data->chg_type =
		(g_usbc_data->muic_data->bc_status & BIT_ChgTyp) >>
		FFS(BIT_ChgTyp);
	g_usbc_data->muic_data->pr_chg_type =
		(g_usbc_data->muic_data->bc_status & BIT_PrChgTyp) >>
		FFS(BIT_PrChgTyp);

	dcdtmo = (g_usbc_data->muic_data->bc_status & BIT_DCDTmo) >>
		 FFS(BIT_DCDTmo);
	g_usbc_data->muic_data->vbusdet = ((g_usbc_data->muic_data->bc_status &
					    BIT_VBUSDet) == BIT_VBUSDet) ?
						  1 :
						  0;
	ccstat = (g_usbc_data->cc_data->cc_status0 & BIT_CCStat) >>
		 FFS(BIT_CCStat);
	...
	if (g_usbc_data->muic_data->vbusdet) {
		...
		schedule_delayed_work(&charger->period_work,
				      msecs_to_jiffies(1000));
		schedule_delayed_work(&charger->batt_notify_work,
...
{
	struct max77729_charger_data *charger = container_of(
		work, struct max77729_charger_data, period_work.work);
	static int wdt_count = 0;
	static int notify_count = 0;
	int batt_curr = 0, batt_volt = 0, batt_ocv = 0, batt_temp = 0,
	    fast_mode = 0, batt_soc, raw_soc, batt_id = 1;
	union power_supply_propval pval = {
		0,
	};
	u8 reg_data;
	...
	if (charger->psy_bms) {
		...
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_CURRENT_AVG, &pval);
		if (rc < 0) {
			pr_err("%s : get POWER_SUPPLY_PROP_CURRENT_AVG fail\n",
			       __func__);
		}
		...
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_VOLTAGE_AVG, &pval);
		if (rc < 0) {
			pr_err("%s : get POWER_SUPPLY_PROP_CURRENT_AVG fail\n",
			       __func__);
		}
		...
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_FASTCHARGE_MODE,
			&pval);
		if (rc < 0) {
		...
		pval.intval = 0;
		rc = power_supply_get_property(
			charger->psy_bms, POWER_SUPPLY_PROP_CAPACITY, &pval);
		...
		if (fast_mode) {
			if (batt_temp >= 150 && batt_temp < 350) {
				...
				max77729_write_word(charger->fg_i2c,
						    ICHGTERM_REG, 0x0400);
			...
			max77729_write_word(charger->fg_i2c, ICHGTERM_REG,
					    0x0160);
		}
		max77729_charger_unlock(charger);
		if (pval.intval) {
			if (fast_mode) {
				...
				if (get_effective_result_locked(
					...
					max77729_update_reg(
						charger->i2c,
						MAX77729_CHG_REG_CNFG_01,
						(0x3
						 << CHG_CNFG_01_CHG_RSTRT_SHIFT),
						CHG_CNFG_01_CHG_RSTRT_MASK);
				} else {
					max77729_update_reg(
						charger->i2c,
...
static int max77729_charger_parse_dt(struct max77729_charger_data *charger)
{
	...
	if (!np) {
		...
		ret = of_property_read_string(
			np, "battery,wireless_charger_name",
	...
	if (!np) {
		...
		ret = of_property_read_u32(np, "charger,fac_vsys",
					   &pdata->fac_vsys);
		if (ret) {
	...
	if (!np) {
		...
		charger->jig_low_active =
			of_property_read_bool(np, "fuelgauge,jig_low_active");
		charger->jig_gpio =
...
static int main_vote_callback(struct votable *votable, void *data, int fcc_ma,
			      const char *client)
{
	/* struct max77729_charger_data *charger = data; */
	union power_supply_propval value;
...
static int fcc_vote_callback(struct votable *votable, void *data, int fcc_ma,
			     const char *client)
{
	struct max77729_charger_data *charger = data;
	/* union power_supply_propval value; */
...
static int chg_vote_callback(struct votable *votable, void *data, int mode,
			     const char *client)
{
	struct max77729_charger_data *charger = data;
	union power_supply_propval value = {
...
static int fv_vote_callback(struct votable *votable, void *data, int fv_mv,
			    const char *client)
{
	/* struct max77729_charger_data *charger = data; */
	union power_supply_propval value;
...
static int max77729_charger_probe(struct platform_device *pdev)
{
	...
	charger_data =
		kzalloc(sizeof(max77729_charger_platform_data_t), GFP_KERNEL);
	if (!charger_data) {
		ret = -ENOMEM;
		goto err_free;
	}
	...
	if (max77729_read_reg(max77729->i2c, MAX77729_PMIC_REG_PMICREV,
			      &reg_data) < 0) {
	...
	(void)debugfs_create_file("max77729-regs", S_IRUGO, NULL,
				  (void *)charger, &max77729_debugfs_fops);
	...
	charger->chgin_ws =
		wakeup_source_register(&pdev->dev, "charger->chgin");
	charger->aicl_ws = wakeup_source_register(&pdev->dev, "charger-aicl");
	charger->wc_current_ws =
		wakeup_source_register(&pdev->dev, "charger->wc-current");
	charger->otg_ws = wakeup_source_register(&pdev->dev, "charger->otg");

	charger_cfg.drv_data = charger;
	charger->psy_chg = devm_power_supply_register(
		&pdev->dev, &max77729_charger_power_supply_desc, &charger_cfg);
	...
	charger->psy_otg = devm_power_supply_register(
		&pdev->dev, &otg_power_supply_desc, &charger_cfg);
...
#if !defined(CONFIG_NOPMI_CHARGER)
	charger->psy_usb = devm_power_supply_register(
		&pdev->dev, &usb_power_supply_desc, &charger_cfg);
	if (IS_ERR(charger->psy_usb)) {
		ret = PTR_ERR(charger->psy_usb);
		pr_err("%s: Failed to Register usb_chg(%d)\n", __func__, ret);
	}

	charger->psy_batt = devm_power_supply_register(
		&pdev->dev, &batt_power_supply_desc, &charger_cfg);
	...
	INIT_DELAYED_WORK(&charger->batt_notify_work,
			  max77729_batt_notify_work);
	INIT_DELAYED_WORK(&charger->period_work, max77729_period_work);
	INIT_DELAYED_WORK(&charger->adapter_change_work,
			  max77729_adapter_changed_work);

	charger->usb_icl_votable = create_votable(
		"USB_ICL", VOTE_MIN, usb_icl_vote_callback, charger);
	...
	charger->fcc_votable =
		create_votable("FCC", VOTE_MIN, fcc_vote_callback, charger);
	...
	charger->fv_votable =
		create_votable("FV", VOTE_MIN, fv_vote_callback, charger);
	...
	if (0) //disable charger irq
		if (charger->pdata->chg_irq) {
			INIT_DELAYED_WORK(&charger->isr_work,
					  max77729_chg_isr_work);
	...
	if (charger->irq_chgin) {
		ret = request_threaded_irq(charger->irq_chgin, NULL,
					   max77729_chgin_irq, 0, "chgin-irq",
					   charger);
		if (ret) {
	...
	ret = request_threaded_irq(charger->irq_bypass, NULL,
				   max77729_bypass_irq, 0, "bypass-irq",
				   charger);
	if (ret < 0)
		pr_err("%s: fail to request bypass IRQ: %d: %d\n", __func__,
		       charger->irq_bypass, ret);
	...
	ret = request_threaded_irq(charger->irq_batp, NULL, max77729_batp_irq,
				   0, "batp-irq", charger);
	if (ret < 0)
	...
	if (charger->pdata->enable_sysovlo_irq) {
		charger->sysovlo_ws =
			wakeup_source_register(&pdev->dev, "max77729-sysovlo");
	...
	if (ret) {
		dev_err(charger->dev,
			"%s : Failed to max77729_chg_create_attrs\n", __func__);
		goto err_atts;
	}
...
static int max77729_charger_remove(struct platform_device *pdev)
{
	...
	if (charger->i2c) {
		...
		max77729_write_reg(charger->i2c, MAX77729_CHG_REG_CNFG_00,
				   reg_data);
		reg_data = 0x0F;
		max77729_write_reg(charger->i2c, MAX77729_CHG_REG_CNFG_09,
				   reg_data);
		reg_data = 0x10;
		max77729_write_reg(charger->i2c, MAX77729_CHG_REG_CNFG_10,
				   reg_data);
		reg_data = 0x60;
		max77729_write_reg(charger->i2c, MAX77729_CHG_REG_CNFG_12,
				   reg_data);
	} else {
