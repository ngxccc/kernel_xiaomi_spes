...
#define BATT_MAX_CHG_VOLT                                                      \
	4460 //new requirement from xiaomi hw, CP should config 4460
...
static int pd_get_batt_current_thermal_level(struct usbpd_pm *pdpm, int *level)
{
	...
	rc = power_supply_get_property(
		pdpm->sw_psy, POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL, &pval);
	if (rc < 0) {
...
static int usbpd_pps_enable_charging(struct usbpd_pm *pdpm, bool en, u32 mV,
				     u32 mA)
{
	int ret, cnt = 0;
	...
	do {
		if (en)
			ret = tcpm_set_apdo_charging_policy(
				pdpm->tcpc, DPM_CHARGING_POLICY_PPS, mV, mA,
				NULL);
		else
...
static int usbpd_select_pdo(struct usbpd_pm *pdpm, u32 mV, u32 mA)
{
	...
	if (NOPMI_CHARGER_IC_MAXIM == nopmi_get_charger_ic_type()) {
		if (!pdpm) {
			...
			return usbpd_select_pdo_maxim(pdpm->apdo_selected_pdo,
						      mV, mA);
		}
	} else {
		if (check_typec_attached_snk(pdpm->tcpc) < 0)
		...
		do {
			...
			cnt++;
		} while (ret != TCP_DPM_RET_SUCCESS &&
			 cnt < PCA_PPS_CMD_RETRY_COUNT);
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_VBUS_ERROR_STATUS, &val);
	if (!ret) {
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_BUS_TEMPERATURE, &val);
	if (!ret)
		pdpm->cp.bus_temp = val.intval;

	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_BATTERY_TEMPERATURE, &val);
	if (!ret)
		pdpm->cp.bat_temp = val.intval;

	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_DIE_TEMPERATURE, &val);
	if (!ret)
		pdpm->cp.die_temp = val.intval;

	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_BATTERY_PRESENT, &val);
	if (!ret)
		pdpm->cp.batt_pres = val.intval;

	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_VBUS_PRESENT, &val);
	if (!ret)
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_ALARM_STATUS, &val);
	if (!ret) {
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_SC_FAULT_STATUS, &val);
	if (!ret) {
...
static void usbpd_pm_update_cp_sec_status(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(
		pdpm->cp_sec_psy, POWER_SUPPLY_PROP_SC_VBUS_ERROR_STATUS, &val);
	if (!ret) {
...
static void usbpd_pm_evaluate_src_caps_maxim(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->pps_supported)
		pr_notice(
			"PPS supported, preferred APDO pos:%d, max volt:%d, current:%d\n",
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt - bq_taper_hys_mv) {
		if (ibus_lmt_change_timer++ > IBUS_CHANGE_TIMEOUT) {
			...
			effective_fcc_taper =
				fcc_vote_val - BQ_TAPER_DECREASE_STEP_MA;
			if (pdpm->fcc_votable) {
	...
	if (pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt)
		...
		step_vbat = pm_config.fc2_steps;
	;
	pr_err("vbat:%d lmt:%d step:%d", pdpm->cp.vbat_volt,
	       pm_config.bat_volt_lp_lmt, step_vbat);
	...
	if (pdpm->cp.ibat_curr < fcc_limit)
		...
		step_ibat = -pm_config.fc2_steps;
	pr_err("ibat:%d lmt:%d step:%d", pdpm->cp.ibat_curr, fcc_limit,
	       step_ibat);
	...
	pr_err("sw_steps:%d hw_steps:%d m_vbush:%d s_vbush:%d", sw_ctrl_steps,
	       hw_ctrl_steps, pdpm->cp.vbus_error_high,
	       pdpm->cp_sec.vbus_error_high);
	...
	if (pdpm->cp.bat_therm_fault) { /* battery overheat, stop charge*/
		...
		pr_notice(
			"bat_ocp_fault:%d, bus_ocp_fault:%d, bat_ovp_fault:%d,bus_ovp_fault:%d\n",
	...
	if (pdpm->cp.ibat_curr > 0 && ibus_total > 0 &&
		...
		fcc_ibatt_diff = (pdpm->cp.ibat_curr > fcc_limit) ?
					 (pdpm->cp.ibat_curr - fcc_limit) :
					 (fcc_limit - pdpm->cp.ibat_curr);
		sicl_ibus_diff = (ibus_total > ibus_limit) ?
					 (ibus_total - ibus_limit) :
					 (ibus_limit - ibus_total);
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_FC2_ENTRY_1:
		...
		pdpm->request_current =
			min(pdpm->apdo_max_curr, pm_config.bus_curr_lp_lmt);
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		pr_err("tune_vbus_retry %d vbus_low:%d vbus_high:%d\n",
		       tune_vbus_retry, pdpm->cp.vbus_error_low,
		       pdpm->cp.vbus_error_high);
		if (pdpm->cp.vbus_error_low ||
		    pdpm->cp.vbus_volt <
			...
			pr_err("vbus low,request_volt:%d,request_curr:%d\n",
			       pdpm->request_voltage, pdpm->request_current);
		} else if (pdpm->cp.vbus_error_high ||
			   pdpm->cp.vbus_volt > pdpm->cp.vbat_volt * 2 +
							BUS_VOLT_INIT_UP +
							200) {
			tune_vbus_retry++;
			pdpm->request_voltage -= 20;
		...
		if (tune_vbus_retry > 30) {
			pr_notice(
				"Failed to tune adapter volt into valid range,charge with switching charger,will try to recover\n");
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
	...
	case PD_PM_STATE_FC2_TUNE:
		...
		if (pm_config.cp_sec_enable && pdpm->cp_sec.charge_enabled &&
		    ...
		    (pdpm->cp.ibus_curr < 750 ||
		     pdpm->cp_sec.ibus_curr < 750)) {
			pr_notice(
				"second cp is disabled due to ibus < 750mA\n");
	...
	case PD_PM_STATE_FC2_EXIT:
		...
		if (recover)
			...
			if (NOPMI_CHARGER_IC_MAXIM ==
			    nopmi_get_charger_ic_type()) {
				//do nothing
			} else if (NOPMI_CHARGER_IC_SYV ==
				   nopmi_get_charger_ic_type()) {
				//do nothing
			} else {
				usbpd_pps_enable_charging(pdpm, false, 5000,
							  3000);
			}
			rc = 1;
		}
...
static int __init usbpd_pm_init(void)
{
	...
	if (!pdpm->cp_psy || (pm_config.cp_sec_enable && !pdpm->cp_sec_psy) ||
	    !pdpm->usb_psy) {
		ret = -EPROBE_DEFER;
		if (probe_cnt <= 10)
			return ret;
