...
int q6usm_run(struct us_client *usc, uint32_t flags, uint32_t msw_ts,
	      uint32_t lsw_ts);
...
struct us_client *q6usm_us_client_alloc(void (*cb)(uint32_t, uint32_t,
						   uint32_t *, void *),
					void *priv);
...
int q6usm_set_us_stream_param(int dir, struct us_client *usc,
			      uint32_t module_id, uint32_t param_id,
			      uint32_t buf_size);
